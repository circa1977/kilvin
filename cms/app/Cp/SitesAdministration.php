...
class SitesAdministration
{
    ...
    */
    public function listSites($message = '')
    {
        ...
        foreach($query as $row)
        {
            ...
            $edit_url   = BASE.'?C=SitesAdministration'.AMP.'M=editSite'.AMP.'site_id='.$row['site_id'];
            ...
            $r .= Cp::tableCell('', Cp::anchor($edit_url, __('sites.edit_site')));
    ...
    * @return string
    */
    public function newSite($site_id = '')
    {
        if ( ! Session::access('can_admin_sites')) {
            return Cp::unauthorizedAccess();
        }
        ...
        $r = Cp::formOpen(
            ['action' => 'C=SitesAdministration'.AMP.'M=updateSite'],
            ['site_id' => $site_id]
        );
        ...
        $r .= '<tr>'.PHP_EOL.
              Cp::tableCell(
                ...
                Cp::required().
                    ...
                    Cp::quickDiv('', __('sites.single_word_no_spaces_with_underscores')),
                '40%'
              ).
        ...
        $r .= '<tr>';
        $r .= '<th class="tableHeadingAlt" style="width: 5%;">ID</th>';
        $r .= '<th class="tableHeadingAlt" style="width: 15%;">Domain</th>';
        $r .= '<th class="tableHeadingAlt" style="width: 20%;">Site URL</th>';
        $r .= '<th class="tableHeadingAlt" style="width: 30%;">CMS Path</th>';
        $r .= '<th  class="tableHeadingAlt" style="width: 30%;">Public Path</th>';
        $r .= '</tr>';

        $domain_placeholder      = 'example.com';
        $site_placeholder        = 'https://example.com';
        $cms_path_placeholder    = base_path();
        ...
        if ($values['site_id'] == '')
        {
            ...
            $r .=  '<tr>'.PHP_EOL
                  ...
                  .'</tr>'.PHP_EOL
                  .'<tr>'.PHP_EOL
                  .     Cp::td('tableHeadingAlt', '', '1').__('sites.weblogs').'</td>'.PHP_EOL
            ...
            $query = DB::table('weblogs')
                ->join('sites', 'sites.site_id', '=', 'weblogs.site_id')
                ->orderBy('site_name')
                ->orderBy('blog_title')
                ->select('blog_title', 'weblog_id', 'site_name')
                ->get();
            ...
            foreach($query as $row)
            {
                ...
                $r .=  '<tr>'.PHP_EOL
                      .     Cp::tableCell('', $row['site_name'].NBS.'-'.NBS.$row['blog_title'])
                      .     Cp::tableCell('', Cp::input_select_header('weblog_'.$row['weblog_id'])
            ...
            $r .=  '<tr>'.PHP_EOL
                  .     Cp::td('tableHeadingAlt', '', '1').__('sites.file_upload_preferences').'</td>'.PHP_EOL
                  .     Cp::td('tableHeadingAlt', '', '1').__('sites.move_options').'</td>'.PHP_EOL
                  .'</tr>'.PHP_EOL;

            $query = DB::table('upload_prefs')
                ->join('sites', 'sites.site_id', '=', 'upload_prefs.site_id')
                ->orderBy('site_name')
                ->orderBy('upload_prefs.name')
                ->select('name', 'id', 'site_name')
                ->get();
            ...
            foreach($query as $row)
            {
                ...
                $r .=  '<tr>'.PHP_EOL
                      .     Cp::tableCell('', $row['site_name'].NBS.'-'.NBS.$row['name'])
                      .     Cp::tableCell('', Cp::input_select_header('upload_'.$row['id'])
                                                      .     Cp::input_select_option('nothing', __('sites.do_nothing'))
            ...
            $r .=  '<tr>'.PHP_EOL
                  .     Cp::td('tableHeadingAlt', '', '1').__('sites.template_variables').'</td>'.PHP_EOL
                  .     Cp::td('tableHeadingAlt', '', '1').__('sites.move_options').'</td>'.PHP_EOL
                  .'</tr>'.PHP_EOL;
            ...
            foreach(Site::sitesList() as $row)
            {
                ...
                $r .=  '<tr>'.PHP_EOL
                      ...
                      .     Cp::tableCell('', Cp::input_select_header('template_variables_'.$row['site_id'])
                                                      .     Cp::input_select_option('nothing', __('sites.do_nothing'))
    ...
    * - Simply calls newSite() with $site_id variable
    *
    * @return string
    */
    function editSite()
    {
        ...
        return $this->newSite(Request::input('site_id'));
    }
        ...
        foreach($input as $key => $value)
        {
            ...
            if (substr($key, 0, strlen('weblog_')) == 'weblog_' && $value != 'nothing' && is_numeric(substr($key, strlen('weblog_'))))
            {
                ...
                if($value == 'duplicate' OR $value == 'duplicate_all')
                {
                    ...
                    // Uniqueness checks
                    foreach(['blog_name', 'blog_title'] AS $check)
                    {
