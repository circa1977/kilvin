...
class Templates
{
    ...
    */
    ...
    {
        if (Site::config("use_category_name") == 'y' AND Site::config("reserved_category_word") != '')
        {
            $this->reserved_names[] = Site::config("reserved_category_word");
        }

        if (Request::input('tgpref') AND Request::input('M') != '')
        {
            Cp::$url_append = AMP.'tgpref='.Request::input('tgpref');
        }

        $this->site_path = CMS_PATH_TEMPLATES.Site::config('site_handle').DIRECTORY_SEPARATOR;
    ...
    */
    ...
    {
        if (Session::userdata('group_id') == 1 or Session::access('can_admin_templates')) {
            return true;
        }
    ...
    */
    public function editTemplateNames($folder = null)
    {
        ...
        Cp::$title  = __('templates.template_names');
        Cp::$crumb  = __('templates.template_names');

        $r = '';

        if (Request::input('msg') == 'success') {
            $r .= Cp::quickDiv('successMessage', __('templates.template_names_updated'));
        }

        $r  .= '<h2>'.sprintf(__('templates.templates_within_x_directory'), $folder).'</h2>';

        $r  .= Cp::formOpen(['action' => 'C=templates'.AMP.'M=updateTemplateNames'])
              .Cp::input_hidden('folder', $folder);

        $r .= '<table class="tableBorder" cellpadding="0" cellspacing="0" style="width:300px;">'
             .'<tr>'.PHP_EOL
             .Cp::tableCell('tableHeadingAlt', __('templates.name_of_template'))
             .Cp::tableCell('tableHeadingAlt', __('templates.type'))
             .'</tr>'.PHP_EOL;
        ...
        foreach ($query as $i => $row)
        {
            ...
            if ($row->template_name == 'index')
            {
                ...
                $t  = __('templates.template_type_'.$row->template_type);
            }
            else
            {
                ...
                foreach($this->template_types as $type) {
                    ...
                    $t .= Cp::input_select_option($type, __('templates.template_type_'.$type), $selected);
                }
    ...
    */
    public function updateTemplateNames()
    {
        ...
        if (!File::isDirectory($folder_path)) {
            return Cp::errorMessage(__('templates.no_folder_exists'));
        }
        ...
        foreach (Request::all() as $k => $val)
        {
            ...
            if ($k == $id.'__template_name')
            {
                ...
                if ($new_name == '') {
                    return Cp::errorMessage(__('templates.missing_name'));
                }

                if ( ! preg_match("#^[a-zA-Z0-9_-]+$#i", $new_name)) {
                    return Cp::errorMessage(__('templates.illegal_characters'));
                }

                if (in_array($new_name, $this->reserved_names)) {
                    return Cp::errorMessage(__('templates.reserved_name'));
                }
    ...
    */
    private function outOfSyncCheck($sync = true)
    {
        ...
        $templates = DB::table('templates')
            ->where('site_id',  Site::config('site_id'))
            ->select('template_name', 'template_type', 'folder', 'updated_at', 'template_id')
            ->orderby('folder')
            ->orderby('template_name')
            ->get()
            ->keyBy('template_id');
    ...
    private function syncFileTemplate($file, $data, $template_id = null)
    {
        ...
        if (!empty($template_id)) {
            DB::table('templates')->where('template_id', $template_id)->update($data);
        }
    ...
    public function templateManager()
    {
        ...
        Cp::$title  = __('cp.design');
        Cp::$crumb = Cp::anchor(BASE.'?C=templates', __('cp.design')).Cp::breadcrumbItem(__('templates.template_management'));
        ...
        $right_links[] = [
            BASE.'?C=templates'.AMP.'M=newTemplateFolderForm',
            __('templates.new_folder')
        ];

        $right_links[] = [
            BASE.'?C=templates'.AMP.'M=templateVariables',
            __('templates.template_variables')
        ];

        $right_links[] = [
            BASE.'?C=Administration'.AMP.'M=config_mgr'.AMP.'P=template_cfg'.AMP.'class_override=templates',
            __('templates.template_preferences')
        ];

        $r  = Cp::header(__('templates.template_management'), $right_links);

        // ------------------------------------
        //  Messages
        // ------------------------------------

        switch (Request::input('msg'))
        {
            case '01' : $message = __('templates.folder_created');
                break;
            case '02' : $message = __('templates.folder_updated');
                break;
            case '03' : $message = __('templates.folder_deleted');
                break;
            case '04' : $message = __('templates.template_created');
                break;
            case '05' : $message = __('templates.template_deleted');
                break;
            default   : $message = '';
                break;
        }
        ...
        {
            $r .= Cp::quickDiv('successMessage', __('templates.search_terms').NBS. Request::input('keywords'));
        }
        ...
        if (Request::has('keywords') && trim(Request::input('keywords')) != '')
        {
            ...
            if (trim($keywords) == '') {
                Cp::$body .= Cp::quickDiv('alert', __('templates.no_results'));
                Cp::$body .= Cp::quickDiv('littlePadding', Cp::anchor(BASE.'?C=templates', __('cp.back')));
                return;
            }
        ...
        if ($query->count() == 0)
        {
            if (isset($keywords))
            {
                Cp::$body .= Cp::quickDiv('alert', __(isset($keywords) ? 'templates.no_results' : 'templates.no_templates_available'));
                Cp::$body .= Cp::quickDiv('littlePadding', Cp::anchor(BASE.'?C=templates', __('cp.back')));
            }
            else
            {
                Cp::$body .= Cp::quickDiv('alert', __('templates.no_templates_available'));
            }
            return;
        }
        ...
        $r .= Cp::tableOpen(array('width' => '99%', 'cellpadding' => '1'))
                .'<tr>'.PHP_EOL
                    ."<td valign='top' style='width:210px; padding-top:1px; text-align:left;'>"
                        .Cp::div('')
                            .Cp::quickDiv('tableHeadingAlt', __('templates.choose_folder'))
        ...
        foreach($query as $e => $row)
        {
            ...
            if (isset($_GET['tgpref']) && is_numeric($_GET['tgpref']) && $_GET['tgpref'] == $this->safeFolder($row->folder))
            {
                $r .= Cp::input_select_option(
                    $this->safeFolder($row->folder),
                    Regex::form_prep($row->folder),
                    'y');
            }
            else
            {
        ...
        $r .= Cp::input_select_footer().
              ...
              '</div>'.PHP_EOL.
              '<br>'.
              Cp::quickDiv('tableHeadingAlt', __('templates.search'))
                    .Cp::div('profileMenuInner')
                    .   Cp::formOpen(['action' => 'C=templates', 'class' => 'cp-form top-margin'])
                    .       Cp::input_text('keywords', '', '20', '120', 'input', '100%')
                    .       Cp::quickDiv('littlePadding', Cp::quickDiv('defaultRight', Cp::input_submit(__('templates.search'))))
                    .   '</form>'.PHP_EOL
                    .'</div>'.PHP_EOL.
              '</td>'.PHP_EOL.
              Cp::tableCell('', '', '8px').
...
$(function() {
    ...
    private function templateRow($row, $current_folder, $sitepath)
    {
        ...
        $edit_url = BASE.'?C=templates'.AMP.'M=editTemplate'.AMP.'id='.$row->template_id.AMP.'tgpref='.$this->safeFolder($row->folder);
        ...
        $t .= Cp::tableCell('', Cp::anchor($viewurl, __('cp.view')));

        $del_url = BASE.'?C=templates'.
            AMP.'M=templateDeleteConfirm'.
            AMP.'id='.$row->template_id.
            AMP.'tgpref='.$this->safeFolder($row->folder);

        $delete =  ($row->template_name == 'index') ? '--' : Cp::anchor($del_url, __('cp.delete'));
    ...
    private function templateFolderOpening($row)
    {
        ...
        $t .= Cp::quickDiv(
            ...
            Cp::anchor(
                BASE.'?C=templates'.
                    AMP.'M=newTemplateForm'.
                    AMP.'folder='.$this->safeFolder($row->folder).
                    AMP.'tgpref='.$this->safeFolder($row->folder),
                __('templates.create_new_template')));
        ...
        $t .= Cp::quickDiv(
            ...
            Cp::anchor(
                BASE.'?C=templates'.
                    AMP.'M=editTemplateNames'.
                    AMP.'folder='.$this->safeFolder($row->folder),
                __('templates.edit_template_names')));
        ...
        if ($row->folder != '/') {
            $t .= Cp::quickDiv(
                    ...
                    Cp::anchor(
                        BASE.'?C=templates'.
                            AMP.'M=editTemplateFolderForm'.
                            AMP.'folder='.$this->safeFolder($row->folder),
                        __('templates.edit_folder')));

            $t .= Cp::quickDiv(
                'littlePadding',
                Cp::anchor(
                    BASE.'?C=templates'.
                        AMP.'M=deleteFolderConfirm'.
                        AMP.'folder='.$this->safeFolder($row->folder),
                    __('templates.delete_folder')));
        }
        ...
        $t .= Cp::table('', '0', '', '100%')
             .'<tr>'.PHP_EOL
             .Cp::tableCell('tableHeading', __('templates.template_name'), '45%')
             .Cp::tableCell('tableHeading', __('templates.template_type'), '15%')
             .Cp::tableCell('tableHeading', __('cp.view'), '15%')
             .Cp::tableCell('tableHeading', __('cp.delete'), '15%')
             .'</tr>'.PHP_EOL;
    ...
    public function editTemplateFolderForm()
    {
        ...
        $title = ($edit == FALSE) ? __('templates.new_folder_form') : __('templates.edit_folder_form');

        // Build the output
        Cp::$title = $title;
        Cp::$crumb = $title;

        Cp::$body = Cp::formOpen(['action' => 'C=templates'.AMP.'M=updateTemplateFolder']);
        ...
        Cp::$body .= Cp::div('box');
         ...
         Cp::$body .=
            Cp::div('paddedWrapper').
                Cp::quickDiv('littlePadding', '<strong>'.__('templates.name_of_folder').'</strong>').
                Cp::quickDiv('littlePadding', __('templates.folder_instructions')).
        ...
        if ($edit === false)
        {
            Cp::$body .= Cp::div('paddedWrapper').
                Cp::quickDiv('littlePadding', Cp::quickDiv('defaultBold', __('templates.duplicate_existing_folder')));
            ...
            Cp::$body .= Cp::input_select_option('false', __('templates.do_not_duplicate_folder'));
        ...
        if ($edit == false) {
            ...
            Cp::$body .= Cp::quickDiv('paddingTop', Cp::input_submit(__('cp.update')));
        }
    ...
    public function updateTemplateFolder()
    {
        ...
        if ( ! $folder = Request::input('folder')){
            return Cp::errorMessage(__('templates.form_is_empty'));
        }
        ...
        if ($edit) {
            if (Request::input('old_name') != $folder and $template_count > 0) {
                return Cp::errorMessage(__('templates.folder_taken'));
            }
        }
        ...
        if (!$edit) {
            ...
            if (Request::has('duplicate_folder')) {
        ...
        return redirect('?C=templates&msg='.$message.$append);
    }
    ...
    public function deleteTemplateFolder()
    {
        ...
        return redirect('?C=templates&msg=03');
    }
    ...
    public function newTemplateForm()
    {
        ...
        Cp::$title = __('templates.new_template_form');
        Cp::$crumb = __('templates.new_template_form');

        $r  = Cp::formOpen(['action' => 'C=templates'.AMP.'M=createNewTemplate']);
        $r .= Cp::input_hidden('folder', $folder);

        $r .= Cp::quickDiv('tableHeading', __('templates.new_template_form'));

        $r .= Cp::div('box');
        $r .= Cp::quickDiv('littlePadding', '<b>'.__('templates.name_of_template').'</b>')
             .Cp::quickDiv('littlePadding', __('templates.folder_instructions'))
             .Cp::quickDiv('', Cp::input_text('template_name', '', '20', '50', 'input', '240px'));


        $r .= Cp::div('littlePadding').'<b>'.__('templates.template_type').'</b>';
        ...
        foreach($this->template_types as $type) {
            ...
            $r .= Cp::input_select_option($type, __('templates.template_type_'.$type), $selected);
        }
        ...
        $r .= Cp::table('tableBorder', '0', '', '100%')
             .'<tr>'.PHP_EOL
             .Cp::td('tableHeadingAlt', '', '3').__('templates.choose_default_data').'</td>'.PHP_EOL
             .'</tr>'.PHP_EOL;
        ...
        $query = DB::table('templates')
            ->select('folder', 'template_name', 'template_id')
        ...
        $r .= '<tr class="has-form">'.PHP_EOL.
                 Cp::tableCell(
                    ...
                    '<label>'.
                        ...
                        ' &nbsp;'.
                        __('templates.an_existing_template').
                    '</label>'
                    ).
                 Cp::tableCell('', $d, '','','left').
             '</tr>'.PHP_EOL.
    ...
    public function createNewTemplate()
    {
        ...
        if ( ! $template_name = Request::input('template_name')) {
            return Cp::errorMessage(__('templates.you_must_submit_a_name'));
        }
        ...
        if ($template_count > 0) {
            return Cp::errorMessage(__('templates.template_name_taken'));
        }

        $template_type = Request::input('template_type');

        if (!in_array($template_type, $this->template_types)) {
            return Cp::errorMessage(__('templates.invalid_template_type'));
        }
        ...
        if (Request::input('data') == 'template')
        {
            ...
            if (!$query) {
                return Cp::errorMessage(__('templates.unable_to_find_duplicate_template'));
            }
        ...
        $append = (Request::input('tgpref')) ? '&tgpref='.Request::input('tgpref') : '';
        return redirect('?C=templates&msg=04'.$append);
    }
    ...
    public function templateDeleteConfirm()
    {
        ...
        Cp::$title  = __('templates.template_delete_confirm');
        Cp::$crumb  = __('templates.template_delete_confirm');

        Cp::$body = Cp::deleteConfirmation(
            [
                'url'       => 'C=templates'.AMP.'M=deleteTemplate',
    ...
    public function deleteTemplate()
    {
        ...
        if (!$query) {
            return Cp::errorMessage(__('templates.unable_to_find_template_in_database'));
        }
        ...
        return redirect('?C=templates&msg=05');
    }
    ...
    public function editTemplate($template_id = '', $message = '')
    {
        ...
        if ($template_id == '') {
            if ( ! $template_id = Request::input('id')) {
                return false;
            }
        }
        ...
        <script type="text/javascript">
            ...
            function viewRevision()
            {
                ...
                else if (id == "clear")
                {
                    ...
                    window.open ("<?php echo BASE.'?C=templates&M=clearRevisions&id='.$template_id.'&Z=1'; ?>" ,"Revision", "width=500, height=260, location=0, menubar=0, resizable=0, scrollbars=0, status=0, titlebar=0, toolbar=0, screenX=60, left=60, screenY=60, top=60");

                    return false;
                }
                else
                {
                    window.open ("<?php echo BASE.'?C=templates&M=viewRevisionHistory&Z=1'; ?>&id="+id ,"Revision");
            ...
            function flipButtonText(which)
            {
                if (which == "clear")
                {
                    document.forms.revisions.submit.value = '<?php echo __('cp.clear'); ?>';
                }
                else
                {
                    document.forms.revisions.submit.value = '<?php echo __('cp.view'); ?>';
                }
            }
        ...
        Cp::$title  = __('templates.edit_template').' | '.$template_name;
        Cp::$crumb  = __('templates.edit_template');
        ...
        $r .= Cp::table('', '', '', '100%').
             '<tr>'.PHP_EOL.
                 Cp::td('tableHeading').
                     '<strong>'.__('templates.last_edit').'</strong> '.$updated_at.
                '</td>'.
            PHP_EOL;
        ...
        $r .= Cp::td('tableHeading')
         ...
         $r .= "<select name='revision_history' class='select' onchange='flipButtonText(this.options[this.selectedIndex].value);'>"
             .PHP_EOL
             .Cp::input_select_option('', __('templates.revision_history'));

        $rquery = DB::table('revision_tracker')
            ->leftJoin('members', 'members.member_id', '=', 'revision_tracker.item_author_id')
            ->where('item_table', 'templates')
            ->where('item_field', 'template_data')
            ->where('item_id', $template_id)
            ->orderBy('tracker_id', 'desc')
            ->select('tracker_id', 'item_date', 'screen_name')
            ->get();
        ...
        if ($rquery->count() > 0)
        {
            ...
            {
                $r .= Cp::input_select_option($row->tracker_id, Localize::createHumanReadableDateTime($row->item_date).' ('.$row->screen_name.')');
            }

            $r .= Cp::input_select_option('clear', __('templates.clear_revision_history'));
        }

        $r .= Cp::input_select_footer()
             .'&nbsp;&nbsp;'
             .Cp::input_submit(__('cp.view'), 'submit', "onclick='return viewRevision();'");
        ...
        $r .= Cp::formOpen(['action' => 'C=templates'.AMP.'M=updateTemplate'])
        ...
        $notelink   = ' <a href="javascript:void(0);" onclick="showhide_notes();return false;"><b>'.__('templates.template_notes').'</b></a>';
        ...
        $r .= $collapse.' '.__('templates.template_notes');
        $r .= '</div>'.PHP_EOL;
        $r .= Cp::div('templatebox');
        $r .= Cp::quickDiv('littlePadding', __('templates.template_notes_desc'));
        ...
        $r .= '</td>'.PHP_EOL
             ...
             .Cp::input_text('columns', Session::userdata('template_size'), '4', '2', 'input', '30px').
                NBS.
                __('templates.template_size')
             .
             '</td>'.PHP_EOL.
             '</tr>'.PHP_EOL.
             '<tr>'.PHP_EOL.
             '<td>'.PHP_EOL.
                 Cp::quickDiv('littlePadding', Cp::input_submit(__('cp.update')).NBS.Cp::input_submit(__('cp.update_and_return'),'return')).
             '</td>'.PHP_EOL.
             '</tr>'.PHP_EOL.
             '</table>'.PHP_EOL.
             '</div>'.PHP_EOL;
    ...
    public function updateTemplate()
    {
        ...
        $message = __('templates.template_updated');
    ...
    public function viewRevisionHistory()
    {
        ...
        Cp::$title  = __('templates.revision_history');
        Cp::$crumb  = __('templates.revision_history');
    ...
    public function clearRevisions()
    {
        ...
        Cp::$body = Cp::quickDiv('defaultCenter', BR.BR.'<b>'.__('templates.history_cleared').'</b>'.BR.BR.BR);

        Cp::$body .= Cp::quickDiv('defaultCenter', "<a href='javascript:window.close();'>".__('cp.close_window')."</a>".BR.BR.BR);
    ...
    public function templateVariables($message = '')
    {
        ...
        Cp::$title = __('templates.template_variables');
        Cp::$crumb = __('templates.template_variables');
        $right_links[] = [
            BASE.'?C=templates'.AMP.'M=editTemplateVariable',
            __('templates.create_new_template_variable')
        ];
        ...
        $query = DB::table('template_variables')
            ...
            ->select('variable_id', 'variable_name', 'variable_data')
            ->get();
        ...
        Cp::$body .= Cp::table('tableBorder', '0', '0', '100%').
                      ...
                      Cp::tableCell('tableHeading',
                                        ($query->count() == 0) ?
                                            array(__('templates.template_variables')) :
                                            array(__('templates.template_variables'),
                                                  __('templates.template_variable_syntax'),
                                                  __('cp.delete')
                                                 )
                                        ).
                      '</tr>'.PHP_EOL;
        ...
        if ($query->count() == 0)
        {
            Cp::$body .= Cp::tableQuickRow(
                '',
                [
                    Cp::quickDiv('littlePadding', Cp::quickDiv('highlight', __('templates.no_template_variables')))
                ]
            );
        }
        else
        {
            foreach ($query as $row)
            {
                Cp::$body .= Cp::tableQuickRow(
                    '',
                    [
                        Cp::quickSpan('defaultBold', Cp::anchor(BASE.'?C=templates'.AMP.'M=editTemplateVariable'.AMP.'id='.$row->variable_id, $row->variable_name)),
                        Cp::quickSpan('defaultBold', '{'.$row->variable_name.'}'),
                        Cp::quickSpan('defaultBold', Cp::anchor(BASE.'?C=templates'.AMP.'M=deleteTemplateVariableConfirm'.AMP.'id='.$row->variable_id, __('cp.delete'))),
                    ]
                );
            }
        }
    ...
    public function editTemplateVariable()
    {
        ...
        Cp::$body  = Cp::quickDiv('tableHeading', __('templates.template_variables'));

        Cp::$body .= Cp::formOpen(
            ['action' => 'C=templates'.AMP.'M=updateTemplateVariable'],
            ['id' => $id]
        );

        Cp::$body .=
            Cp::div('box').
                Cp::heading(__('templates.variable_name'), 5).
                Cp::quickDiv('littlePadding',  __('templates.folder_instructions')).
                Cp::quickDiv('littlePadding', Cp::input_text('variable_name', $variable_name, '20', '50', 'input', '240px')).
                Cp::heading(BR.__('templates.variable_data'), 5).
        ...
        if (!Request::has('id')) {
            Cp::$body .= Cp::input_submit(__('cp.submit'));
        }
        else {
            Cp::$body .= Cp::input_submit(__('cp.update'));
        }
    ...
    public function updateTemplateVariable()
    {
        ...
        if (!Request::has('variable_name') or !Request::has('variable_data')) {
            return Cp::errorMessage(__('templates.all_fields_required'));
        }
        ...
        if (Request::has('id'))
        {
            DB::table('template_variables')
                ->where('variable_id', Request::input('id'))
                ->update(
                    [
            ...
            $msg = __('templates.global_var_updated');
        }
        ...
        if (!Request::has('id'))
        {
            ...
            if ($count > 0) {
                return Cp::errorMessage(__('templates.duplicate_var_name'));
            }
            ...
            $msg = __('templates.template_variable_created');
        }
    ...
    public function deleteTemplateVariableConfirm()
    {
        ...
        Cp::$title  = __('templates.delete_template_variable');
        Cp::$crumb  = __('templates.delete_template_variable');
        ...
        Cp::$body = Cp::deleteConfirmation(
            array(
                'url'       => 'C=templates'.AMP.'M=deleteTemplateVariable',
    ...
    public function deleteTemplateVariable()
    {
        ...
        return $this->templateVariables(__('templates.variable_deleted'));
    }
}
