...
class Controller extends BaseController
{
    ...
    public function all()
    {
        ...
        // But only if they are not a Super Admin, as they cannot be banned

        if (Session::userdata('group_id') != 1 AND Session::banCheck('ip')) {
            return Cp::unauthorizedAccess(__('not_authorized'));
        }

        // ------------------------------------
        //  Temp - Some of our forms use ALL $_POST data for an insert
        // ------------------------------------
        ...
        $class_map = [
            ...
            'Sites'                 => ['Sites'],
            'SitesAdministration'   => ['SitesAdministration'],
        ];
        ...
        if (!Auth::check() and !in_array(Request::input('C'), $forgot_related))
        {
        ...
        else
        {
            ...
            if ( ! in_array(Request::input('C'), array_keys($class_map)))
            {
            ...
            {
                $class =  $class_map[Request::input('C')][0];

                $method = ( ! isset($class_map[Request::input('C')][1])) ? '' : $class_map[Request::input('C')][1];
            }
        }
