	...
	*/
    ...
    public function toMail($notifiable)
    {
        $entry = DB::table('weblog_entries')
            ...
            ->where('weblog_entries.entry_id', $this->entry_id)
            ->first();
        ...
        $vars['subject']         = __('emails.new_weblog_entry_posted');

        $vars['greeting']        = 'Hello!';
        $vars['introLines'][]    = 'A new entry has been posted on '.Site::config('site_name');
        $vars['actionText']      = $entry->title;
        $vars['actionUrl']       = Site::config('cp_url').'?C=edit&M=edit_entry&weblog_id='.$entry->weblog_id.'&entry_id='.$entry->entry_id;

        return (new MailMessage)
            ->from($vars['notification_sender_email'], $vars['site_name'])
            ->subject(__('emails.new_weblog_entry_posted'))
            ->view('emails.base', $vars);
    }
}
