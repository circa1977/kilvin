...
class TwigServiceProvider extends ViewServiceProvider
{
    ...
    protected function registerLoaders()
    {
        ...
        // Middleware disabled occasionally and console does not have this, yet
        if (defined('REQUEST')) {
            if (REQUEST === 'CP') {
                $paths[] = resource_path('views/cms');
            }

            if (REQUEST === 'SITE') {
                $paths[] = base_path('templates/_global'); // using prependLocation() so goes first
                $paths[] = base_path('templates/'.Site::config('site_handle'));
            }

            if (REQUEST === 'INSTALL') {
                $paths[] = resource_path('views/cms');
                $paths[] = resource_path('views/installer');
            }
        } else {
            $paths[] = resource_path('views');
        }

        $paths = array_merge($paths, config('view.paths', []));
