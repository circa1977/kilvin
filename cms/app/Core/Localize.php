...
class Localize
{
    ...
    * Create a Carbon object for us
     ...
     * @return Carbon\Carbon
     */
    public static function createHumanReadableDateTime($now = null, $seconds = false)
    {
        ...
        if (func_num_args() != 2 && Site::config('include_seconds') == 'y') {
            $seconds = true;
        }

        $object->timezone = Site::config('site_timezone');
        $string = $object->toDateTimeString();
    ...
    public static function humanReadableToUtcCarbon($datestr = '', $timezone = null)
    {
        ...
        if ( ! preg_match('/^[0-9]{2,4}\-[0-9]{1,2}\-[0-9]{1,2}\s[0-9]{1,2}:[0-9]{1,2}(?::[0-9]{1,2})?(?:\s[AP]M)?$/i', $datestr)) {
            return __('publish.invalid_date_formatting');
        }
