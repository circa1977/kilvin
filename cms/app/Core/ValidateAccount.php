...
class ValidateAccount
{
    ...
    private function checkForPassword()
    {
        if (empty($this->current_password)) {
            return $this->errors[] = __('account.missing_current_password');
        }

        if ($this->validateCredentials($this->member_id, $this->current_password)) {
            $this->errors[] = __('account.invalid_password');
        }
    }
    ...
    public function validateScreenName()
    {
        ...
        {
            return $this->errors[] = __('account.disallowed_screen_chars');
        }
        ...
        if (!empty($this->current_screen_name))
        {
            if ($this->current_screen_name != $this->screen_name)
            {
                ...
                if ($this->enable_log == true) {
                    $this->log_msg[] = __('account.screen_name_changed').NBS.$this->screen_name;
                }
            }
        }
        ...
        if ($type == 'new')
        {
            ...
            {
                return $this->errors[] = __('account.screen_name_taken');
            }
            ...
            if (strtolower($this->current_screen_name) != strtolower($this->screen_name))
            {
                ...
                if ($count > 0) {
                    $this->errors[] = __('account.screen_name_taken');
                }
            }
        }
    }
    ...
    public function validatePassword()
    {
        ...
        if (empty($this->password) or empty($this->password_confirm)) {
           return $this->errors[] = __('account.missing_password');
        }
        ...
        if (strlen($this->password) < $len) {
            return $this->errors[] = str_replace('%x', $len, __('account.password_too_short'));
        }
        ...
        if (strlen($this->password) > 50) {
            return $this->errors[] = __('account.password_too_long');
        }
        ...
        if ($lc_user == $lc_pass or $lc_user == strrev($lc_pass) or $lc_user == $nm_pass or $lc_user == strrev($nm_pass)) {
            return $this->errors[] = __('account.password_based_on_email');
        }
        ...
        if ($this->password != $this->password_confirm) {
            return $this->errors[] = __('account.missmatched_passwords');
        }
    }
    ...
    public function validateEmail()
    {
        ...
        if (empty($this->email)) {
            return $this->errors[] = __('account.missing_email');
        }
        ...
        if ( ! Regex::valid_email($this->email)) {
            return $this->errors[] = __('account.invalid_email_address');
        }
        ...
        if (!empty($this->current_email))
        {
            ...
            {
                if ($this->enable_log == true) {
                    $this->log_msg = __('account.email_changed').NBS.$this->email;
                }
        ...
        if ($type == 'new')
        {
            ...
            if (Session::banCheck('email', $this->email)) {
                return $this->errors[] = __('account.email_taken');
            }
            ...
            if ($count > 0) {
                $this->errors[] = __('account.email_taken');
            }
        }
    }
