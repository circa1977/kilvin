...
{exp:weblogs:entries weblog="{master_weblog_name}" limit="10" rdf="off" dynamic_start="on" disable="member_data"}
    <item>
...
<feed xmlns="http://www.w3.org/2005/Atom" xml:lang="{weblog_language}">
    ...
    <updated>{updated_at | date('%Y-%m-%dT%H:%i:%sZ')}</updated>
...
{exp:weblogs:entries weblog="{master_weblog_name}" limit="15" rdf="off" dynamic_start="on" disable="member_data"}
    <entry>
      <title>{exp:xml_encode}{title}{/exp:xml_encode}</title>
      ...
      <updated>{updated_at | format('%Y-%m-%dT%H:%i:%sZ')}</updated>
      <author>
            <name>{author}</name>
            <email>{email}</email>
