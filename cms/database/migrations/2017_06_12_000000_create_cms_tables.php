...
class CreateCmsTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('sites', function($table)
        {
            ...
            $table->text('site_preferences')->nullable();
            $table->text('member_preferences')->nullable();
            $table->text('template_preferences')->nullable();
            $table->text('weblog_preferences')->nullable();
        });
        ...
        Schema::create('email_console_cache', function($table)
        {
            $table->increments('cache_id');
            $table->timestamp('cache_date')->index();
            $table->unsignedInteger('member_id')->index();
            $table->string('member_name', 50);
            $table->string('ip_address', 45)->index();
            $table->string('recipient', 75);
        ...
        // Email Tracker
        // This table is used by the Email plugin for flood control.
        Schema::create('email_tracker', function($table)
        {
            $table->increments('email_id');
            $table->timestamp('email_date');
            $table->string('sender_ip', 45)->index();
            $table->string('sender_email', 75)->index();
            $table->string('sender_screen_name', 50);
            $table->unsignedInteger('number_recipients')->default(1);
        });
        ...
        Schema::create('members', function($table)
        {
            ...
            $table->timestamp('last_view_bulletins')->nullable();
            $table->timestamp('last_bulletin_date')->nullable();
            ...
            $table->char('time_format', 2)->default('us');
        ...
        Schema::create('member_homepage', function($table)
        {
            $table->increments('member_homepage_id');
            $table->unsignedInteger('member_id')->index();

            $table->char('recent_entries', 1)->default('l');
            $table->unsignedSmallInteger('recent_entries_order')->default(1);

            $table->char('recent_members', 1)->default('n');
            $table->unsignedSmallInteger('recent_members_order')->default(1);

            $table->char('site_statistics', 1)->default('r');
            $table->unsignedSmallInteger('site_statistics_order')->default(1);

            $table->char('member_search_form', 1)->default('n');
            $table->unsignedSmallInteger('member_search_form_order')->default(1);

            $table->char('notepad', 1)->default('r');
            $table->unsignedSmallInteger('notepad_order')->default(1);

            $table->char('bulletin_board', 1)->default('r');
            $table->unsignedSmallInteger('bulletin_board_order')->default(1);
        });
        ...
        {
            $table->increments('m_field_id');
            $table->string('m_field_name', 32);
            $table->string('m_field_label', 50);
            $table->text('m_field_description');
            $table->string('m_field_type', 12)->default('text');
            $table->text('m_field_list_items');
            $table->unsignedSmallInteger('m_field_ta_rows')->default(8);
            $table->unsignedSmallInteger('m_field_maxl')->nullable();
            $table->string('m_field_width', 6); // Should this be an integer?
            $table->char('m_field_search')->default('y');
            $table->char('m_field_required')->default('n');
            $table->char('m_field_public')->default('y');
            $table->char('m_field_reg')->default('n');
            $table->unsignedSmallInteger('m_field_order')->default(1);
        });
        ...
        Schema::create('weblogs', function($table)
        {
            ...
            $table->string('blog_name', 40);
            $table->string('blog_title', 100);
            $table->string('blog_url', 100);
            $table->string('blog_description')->default('');
            ...
            $table->char('show_options_cluster', 1)->default('y');
            $table->char('show_show_all_cluster', 1)->default('y');
            $table->char('show_author_menu', 1)->default('y');
            $table->char('show_status_menu', 1)->default('y');
            $table->char('show_categories_menu', 1)->default('y');
            $table->char('show_date_menu', 1)->default('y');
        ...
        Schema::create('weblog_fields', function($table)
        {
            ...
            $table->unsignedTinyInteger('field_ta_rows')->default(8);
            $table->unsignedSmallInteger('field_maxl')->nullable();

            $table->char('field_required', 1)->default('n');
            $table->char('field_text_direction', 3)->default('ltr');
            $table->char('field_search', 1)->default('n');
            $table->char('field_is_hidden', 1)->default('n');
        });
        ...
        Schema::create('search', function($table)
        {
            ...
            $table->unsignedTinyInteger('per_page');
            $table->mediumText('query');
            $table->mediumText('custom_fields');
            $table->string('result_page');
        });

        Schema::create('member_bulletin_board', function($table)
        {
            $table->increments('bulletin_id');
            $table->unsignedInteger('sender_id')->index();
            $table->unsignedInteger('bulletin_group')->index();
            $table->timestamp('bulletin_date');
            $table->string('hash', 20)->index();
            $table->timestamp('bulletin_expires')->nullable();
            $table->text('bulletin_message');
        });
    }
